...
5. `src/index.ts` — call `server.registerTool(name, { description, inputSchema, annotations: { title, ...READONLY_ANNOTATIONS } }, handler)`, passing the zod schema straight through as `inputSchema` (it doubles as the wire-format JSON Schema, generated automatically) and wrapping the handler in `withErrorHandling` unless it needs a non-JSON content block (see `get_radar_image`, which returns an `image` block alongside `text` and so wires its own try/catch).

Each tool module is a thin adapter: the SDK validates `inputSchema` automatically before the handler runs, and all HTTP/data-shaping logic lives in `smhi-client.ts`. Tool handlers just call the client and wrap the result with `location`/`period` metadata.
...
**Error handling:** all client functions throw `SMHIClientError` (with optional `statusCode`) on network failure, non-OK responses, or missing data. Each tool handler in `src/index.ts` catches everything (via the `withErrorHandling` wrapper, or its own try/catch for `get_radar_image`) and returns `{ content: [...], isError: true }` rather than throwing across the MCP boundary — never let an exception escape a `registerTool` handler. (The SDK's own `tools/call` dispatch also catches unhandled throws and converts them to an `isError` result, but the explicit wrapper keeps the `Error: <message>` text format consistent with what the tool descriptions document.)
